...
- **(Fase 6) Criar produto novo** pelo botão "Novo produto" (nome, código SKU, EAN, unidade, valor de venda e categoria) — desde 23/07/2026 o produto nasce **só no app** (o Conta Azul virou somente leitura e não recebe mais cadastros). **Desde 09/2026 a categoria é OBRIGATÓRIA** (é ela que diz se o produto controla estoque) — sem categoria válida o servidor recusa a criação
- **(09/2026) Produto nascido de uma nota de entrada**: quando o produto foi criado pela conferência de Notas Recebidas (destino "➕ Criar produto novo") ou por "Enviar para Produtos" (PCP → Itens), a ficha mostra **"📄 Cadastrado a partir da NF-e {número} ({fornecedor}) em {data} como '{nome na nota}'"** e **"🏷️ Fornecedores que já vieram com este produto"** (o de-para por fornecedor + código do produto na nota). Produto cadastrado à mão não mostra essas linhas
- **(09/2026) EAN e NCM agora são editáveis** na ficha do produto (antes só vinham do cadastro original/XML e eram somente leitura)
