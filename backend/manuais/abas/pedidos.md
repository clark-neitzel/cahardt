...
7. Adicione os produtos e quantidades
   ...
   - No topo da lista de produtos, quando o cliente tem histórico e o catálogo já carregou, aparece a faixa **"Repetir último pedido (N itens · última compra dd/mm)"** (enquanto o catálogo troca — ex.: acabou de escolher Pedido Especial — a faixa fica cinza, escrito "Carregando catálogo…", até não vender pelo catálogo errado)
   - Ao tocar, abre o modal **"Conferir antes de adicionar"** — nada entra no carrinho ainda. Cada linha mostra o produto, a quantidade da última compra (**editável**), a data da última compra e o preço atual (mesma regra do carrinho: último preço pago pelo cliente, senão tabela + acréscimo da condição); um checkbox marcado por padrão seleciona o item. Produto com estoque insuficiente para a quantidade aparece com aviso âmbar mas continua marcável; produto que **saiu do catálogo** aparece cinza, sem checkbox (nunca entra). "Selecionar todos" marca/desmarca de uma vez, e o total estimado dos itens marcados fica visível no rodapé
   - Se o carrinho já tinha itens antes de abrir o modal, um aviso amarelo avisa que ele será **substituído**, e o botão de confirmar já vem escrito "Substituir carrinho por N itens" (em vez de "Adicionar N itens ao carrinho")
   - Só ao tocar em **"Adicionar/Substituir"** os itens marcados entram no carrinho, pela mesma função usada ao adicionar item na mão — preço, trava de estoque e promoção continuam valendo do mesmo jeito. O pedido **nunca é enviado sozinho**: o vendedor revisa (e pode editar/remover itens) antes de Salvar/Enviar, como em qualquer pedido
