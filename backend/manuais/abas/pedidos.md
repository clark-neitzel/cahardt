...
7. Adicione os produtos e quantidades
   ...
   - No topo da lista de produtos, quando o cliente tem um **último pedido de verdade** (não bonificação, não cancelado/excluído) e o catálogo já carregou, aparece a faixa **"Repetir último pedido (#número · dd/mm · N itens)"** (enquanto o catálogo troca — ex.: acabou de escolher Pedido Especial — a faixa fica cinza, escrito "Carregando catálogo…", até não vender pelo catálogo errado)
   - **A partir de 09/2026, "Repetir" repete o PEDIDO INTEIRO anterior** (não mais "a compra mais recente de cada produto que o cliente já levou na vida" — isso trazia produto que o cliente não compra mais). Ao tocar, o sistema confere se dá para repetir exatamente como foi: mesmo tipo (normal/especial), mesma condição de pagamento (se ainda está liberada para o cliente), todo produto ainda no catálogo, estoque suficiente e — o ponto central — **o preço de hoje precisa bater exatamente com o preço do último pedido** (mesma regra de preço usada ao adicionar item na mão: tabela + acréscimo da condição, respeitando promoção e piso de desconto do vendedor)
   - **Sem divergência**: abre "Repetir pedido #N de dd/mm" com a lista **somente leitura** (produto, quantidade, preço unitário, subtotal) e o total. Se o carrinho já tinha itens, um aviso amarelo avisa que será **substituído**. Ao tocar em **"Repetir pedido"**, a condição de pagamento muda para a do último pedido e o carrinho é substituído pelos mesmos itens/quantidades/preços — sem checkbox, sem editar nada nessa hora (quem quiser diferente ajusta no carrinho depois, como qualquer item). O pedido **nunca é enviado sozinho**: o vendedor revisa e clica em Salvar/Enviar como em qualquer pedido
   - **Com divergência**: abre "Não é possível repetir o pedido #N" com a lista em vermelho dos motivos (produto fora do catálogo, sem estoque, condição não disponível, ou o preço de hoje diferente do último pedido — cada linha nomeia o produto/motivo) e a instrução "Para não errar o pedido, monte-o manualmente." **Nada entra no carrinho**; só um botão "Entendi" para fechar
