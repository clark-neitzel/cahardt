...
- Criar uma conta a pagar: fornecedor, descrição, categoria de despesa, número da nota, competência, observações e parcelas (valor + vencimento de cada uma). **A conta fica só no app** (não é mais enviada ao Conta Azul)
  - O campo **Categoria** é uma lista de escolha (não aceita nome digitado livre). Se a categoria que você precisa não estiver lá, crie em **Financeiro → Categorias de Despesa**, card "Nova categoria" — ela aparece nessa lista logo em seguida
...
- **Lançar os produtos da compra junto com a despesa manual (opcional)**: na tela de Nova Despesa (entre as Observações e as Parcelas) há a seção **"Produtos comprados"** — clique em "Lançar os produtos desta compra", busque o produto **do catálogo de produtos** (insumos do PCP não aparecem nessa busca, igual ao de-para das Notas Recebidas) e informe a **quantidade** (na nossa unidade) e o **valor unitário OU o valor total** — preenchendo um, o outro é calculado sozinho. Fornecedor, categoria e produto usam **busca com filtro** (combobox digitável, não lista gigante). Ao criar a despesa, cada produto:
  ...
  - recalcula o **custo** pela regra das compras recentes — a **média das compras válidas mais recentes que cobrem o estoque atual** (a compra que fecha o estoque entra inteira; sem compra válida o custo é mantido). Produto → custo manual; insumo PCP → custo unitário das receitas;
...
- **Cancelar a despesa devolve o estoque** (estorna as entradas) **e recalcula o custo** pela regra das compras recentes (a média das compras válidas que ainda cobrem o estoque — não "volta ao valor de antes"). Quando **não sobra nenhuma compra válida**, o app **mantém o custo e avisa** para conferir o Custo Manual do produto — nunca zera calado.
...
- **Importação do Conta Azul — DESLIGADA (09/2026):** o botão "Importar do Conta Azul" (CSV) e a sincronização automática de 3h que trazia despesas lançadas direto no CA foram removidos (o dono confirmou que não usa mais nenhum dos dois — plano de remoção do CA, `docs/plano-remocao-conta-azul.md`). As contas que já tinham chegado por essas duas vias continuam na base com origem **IMPORTADO_CA**, e as parcelas mapeadas ao CA (`idParcelaCA`) continuam recebendo a baixa pela conferência de 30 minutos (ver item abaixo). O que já foi importado não some; só não chega mais gente nova por essas duas portas.
...
- Cancelar uma conta (só se não tiver pagamento registrado; estorne antes se precisar). Se a despesa manual tinha **produtos lançados**, o cancelamento **devolve o estoque e recalcula o custo** automaticamente (ver detalhes na seção "Lançar os produtos da compra", acima)
...
A importação de histórico do CA (CSV e sincronização automática de 3h) foi **desligada em 09/2026** — não é mais necessária, já que despesa nova nasce só no app.
