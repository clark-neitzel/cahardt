...
6. **Finaliza:** o pedido é salvo e aparece a tela "Pedido registrado!" com o botão **"Enviar pedido pelo WhatsApp"** (o cliente pode, opcionalmente, abrir a conversa com a loja por ali). Além disso, **assim que o pedido é finalizado, o WhatsApp da Hardt envia automaticamente uma mensagem de confirmação para o celular do cliente**, com o resumo do pedido (itens, data/horário, retirada ou entrega e total). Se o número for novo/corrigido, o pedido é marcado como **"celular alterado"** para a equipe atualizar no cadastro. O pagamento é combinado depois (pix ou na entrega).
...
### 1. Pedidos
Fila de pedidos vindos do site, **do mais recente para o mais antigo**, no **mesmo desenho da aba de Pedidos** (desde 14/09/2026): barra de busca em card branco (busca por **nome, razão social, nome fantasia, cidade, CPF ou CNPJ** e telefone, com botão de atualizar), a linha **"Pendentes:"** com as contagens de Aguardando / Sem cadastro (clicar filtra), **chips de status com contagem** (Todos / Aguardando / Sem cadastro / Convertidos / Recusados / Cancelados) e os pedidos **em linhas, um abaixo do outro**, dentro de um card branco. Cada linha mostra número, cliente e total; **Entrega** ou **Retirada** com data e horário, quando o pedido foi feito e "por: nome" quando o nome do site difere do cadastro; cidade; CPF/CNPJ e telefone; caixas; e os selos (status, avisos de sem cadastro / celular alterado, **→ Pedido #N**) com o botão **Detalhes**. A lista **atualiza sozinha a cada 45 segundos**. Pedidos novos ganham o selo dourado **NOVO** no início da linha. Quando o pedido gerado é **excluído no sistema**, o pedido do site vira **Cancelado** automaticamente.
...
- **Pedidos "Sem cadastro"** aparecem destacados em vermelho. Antes de aprovar, é preciso **vincular** o pedido a um cliente do app (cadastre no Conta Azul, sincronize, busque e vincule). A busca aceita **2+ letras** do nome, razão, fantasia ou CPF/CNPJ e mostra as sugestões abaixo do campo — clique na sugestão para vincular.
- **Cliente vinculado:** quando o pedido está ligado a um cadastro do app, a linha e o detalhe passam a mostrar o **nome (razão social) e o CPF/CNPJ do cadastro vinculado** (e não o nome que o cliente digitou no site). Se o nome do site for diferente, ele aparece numa linha discreta **"Pedido feito por …"**, para não se confundir com a razão social do cadastro.
