...
2. **Catálogo personalizado:** para clientes com cadastro, aparece primeiro **"Você sempre pede"** — os produtos das **últimas 5 compras** do cliente (todos os itens, sem limite). Não conta pedidos de **bonificação** nem pedidos excluídos, e **desconta devoluções**: produto totalmente devolvido e que não aparece em nenhum dos outros pedidos não entra na lista. O produto também só aparece se estiver **cadastrado no site** (em "Produtos no site"). Depois vem o catálogo **separado por categoria** (cada categoria como uma seção, na ordem definida no admin), com os produtos em **ordem alfabética** dentro de cada categoria. A vitrine usa o **mesmo visual claro do Kit Festa** (cards cor de creme) e os filtros por **categoria comercial** aparecem como **pílulas** no topo (sem campo de busca). O Kit Festa **não** entra no catálogo de congelados.
   ...
   - **Sem estoque = "Indisponível":** produto do site com **estoque disponível zerado** aparece com a etiqueta vermelha **"Indisponível"**, foto acinzentada e **sem o botão de adicionar** (mostra "Sem estoque" no lugar) — o cliente não consegue comprar. Se o produto já estava no carrinho (salvo de antes) ou o estoque zerar no meio do pedido, o carrinho avisa e **bloqueia o envio** até o cliente remover o item; o servidor também recusa o pedido por segurança. Assim que o estoque volta, o produto libera sozinho — basta o app registrar a entrada (não há botão pra isso no admin do site). O botão **"Repetir último pedido"** também pula itens indisponíveis.
...
4. **Carrinho (gaveta lateral):** mostra a **condição de pagamento padrão** do cliente (o site usa só a padrão), a **data de entrega**, telefone e observações. Tem botão **"Limpar"** (esvaziar o carrinho). O carrinho fica **salvo no navegador** (não se perde ao atualizar a página): guarda só as quantidades — o **preço é sempre lido do catálogo atual** (mudança de preço reflete sozinha) e itens que **saíram do site/foram inativados são removidos** automaticamente ao recarregar.
   - **Entrega ou retirada:** o cliente escolhe **Entrega** ou **Retirar na loja**.
     - **Entrega:** o site sugere a **próxima data do dia regular** do cadastro do cliente (ex.: cadastro com Quarta → sugere a próxima quarta; nunca hoje). Pode **trocar a data** num **calendário** — não pode ser hoje nem fim de semana (sábado/domingo só se o admin liberar). Se a data **não for o dia regular** do cliente (ou cliente sem dia / visitante), vira **"encaixe"**: o cliente é avisado de que a equipe vai **confirmar a viabilidade** da entrega naquela data.
     - **Retirada:** o cliente escolhe a data no **calendário** seguindo as mesmas regras (não hoje, fim de semana só se liberado), **sem** considerar o dia de entrega do cadastro e **sem** virar encaixe. Mostra o endereço da loja.
     - É obrigatório escolher uma data para enviar o pedido.
...
9. **Falar com vendedor (integração com o Bot Hardt):** no topo do catálogo há uma pílula de WhatsApp que abre a conversa **no número oficial da empresa**, mas endereçada ao vendedor certo — o bot de WhatsApp (Painel Hardt) lê um marcador no fim da mensagem (`[Vendedor Hardt: Nome]`), entrega a conversa direto para a pessoa e pausa a Ana. Como decide o que mostrar:
   - Cliente logado cujo vendedor tem o **"Nome usado no Bot Hardt"** preenchido (aba Usuários) **e** está na lista oficial do bot → botão direto **"Falar com [nome]"**.
   - Vendedor do cliente sem vínculo ou fora da lista → aviso "Seu vendedor atual ainda não atende por este canal" + escolha entre os vendedores disponíveis.
   - Cliente novo/sem vendedor (inclusive visitante) → escolhe um vendedor da lista. Na página `/inicio`, o **botão flutuante de WhatsApp** abre essa mesma escolha.
   - Lista vazia ou bot fora do ar → botão único **"Falar com a equipe"** (conversa cai na recepção normal).
   - A lista oficial vem do bot (`/api/congelados-publico/vendedores-site`, renovada a cada 5 min) e mostra **só nome e setor** — telefone pessoal do vendedor nunca aparece. O cliente ainda precisa tocar em **Enviar** no WhatsApp.
...
> **Pedidos pelo WhatsApp (assistente "Ana"):** além dos pedidos feitos no site, esta mesma fila também recebe os pedidos que o **assistente de IA do WhatsApp** fecha com o cliente. Eles chegam iguais aos do site (nascem **Aguardando**, ou **Sem cadastro** se o telefone for de cliente novo) e vêm com **"[WhatsApp IA]"** no início das observações. O cliente é identificado pelo **telefone** e o **preço é o mesmo que o vendedor veria** (recalculado no servidor). O tratamento é o mesmo: conferir e **Aprovar** (escolhendo tipo e data).
