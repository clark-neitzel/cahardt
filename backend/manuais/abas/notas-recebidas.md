...
- **Gerar a conta a pagar** a partir da nota:
  ...
  - **Categoria da despesa por item (Contas a Pagar / DRE)**: pode escolher uma **categoria padrão** para a nota inteira e, se quiser, uma **categoria diferente item a item**. Itens sem categoria própria usam a padrão. **Não confundir com a categoria do PRODUTO** — essa é outra coisa, escolhida dentro do modal "Criar produto novo" (ver abaixo).
  ...
  - **Cada item da nota tem TRÊS destinos, e só três (09/2026):**
    1. **🔗 Vincular a um produto** — abre a busca de "Nosso produto", que procura no **catálogo de Produtos** e nos **insumos do PCP já cadastrados** (grupo rotulado "Insumo do PCP (subproduto/espelho)", para não confundir com produto de verdade). Se a unidade da nota for diferente da nossa, pede a **conversão de quantidade** — o vínculo e a conversão ficam **memorizados por fornecedor + código do produto na nota** (e código de barras, quando houver), então da 2ª nota em diante já vem preenchido.
    2. **➕ Criar produto novo** — abre o modal **"Criar produto a partir da nota"**: nome (vem pré-preenchido com o texto da nota, editável — o texto original fica guardado como "nome na nota"), **categoria** (obrigatória, da lista de Categorias de Estoque — é ela que diz se o produto controla estoque), categoria comercial (opcional, só para o que se vende), EAN e NCM (vêm do XML, editáveis), unidade + conversão, e controle de estoque (Segue a categoria / Controlar sempre / Nunca controlar). O produto **nasce de verdade no cadastro de Produtos** — não é mais um "item PCP solto": se a categoria for **Matéria-Prima ou Embalagem**, o insumo do PCP nasce **sozinho, automaticamente**, já ligado ao produto (não existe mais escolher "tipo do PCP" na conferência).
    3. **🚫 Não é estoque** — motivos em pílula: **Serviço**, **Frete**, **Imposto/taxa**, **Consumo imediato**, **Imobilizado** (novo, 09/2026 — separa a compra de bem do imobilizado igual, freezer/painel/móveis, sem precisar cadastrar produto antes) ou **Outro** (exige uma explicação escrita).
  - **TODO item precisa de destino.** Em nota que não é NFS-e, cada item da nota tem que ter um dos três destinos acima. Enquanto sobrar item sem destino, o app **recusa** a conferência e mostra quais itens faltam. Item deixado em branco nunca soma estoque, nunca gera custo e nunca entra no histórico de compras.
  ...
  - O sistema também **lembra a categoria da despesa escolhida por produto do fornecedor** (e por prestador, na NFS-e), mesmo sem vínculo de produto — na próxima nota do mesmo fornecedor a categoria já vem sugerida
  - **Entrada de estoque respeita a categoria do produto (09/2026):** produto de categoria que **não controla estoque** (ex.: Uso e Consumo) vira **só despesa** ao ser conferido — não soma quantidade nem mexe em custo médio, mas o histórico de compras (fornecedor/preço) é gravado do mesmo jeito. Produto de categoria que controla estoque continua somando normal. Produto sem categoria é tratado como "não controla" e o app avisa na tela para você conferir a categoria dele.
...
- Deixa você trocar o **produto vinculado** e a **conversão de quantidade** de cada item (inclusive vincular um item que não tinha entrado, ou desvincular um que entrou errado). Também dá para **"Criar produto novo"** aqui, com o mesmo modal da conferência (categoria obrigatória, EAN/NCM, controle de estoque).
