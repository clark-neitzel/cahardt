...
- **Gerar a conta a pagar** a partir da nota:
  ...
  - **Observações vão para a descrição da despesa no CA.** O que você digitar no campo **Observações** é anexado à **descrição** da despesa no Conta Azul (fica `NF-e 123 — Fornecedor — sua observação`), mantendo o número da nota e o fornecedor na frente (que o app usa para conciliar e não duplicar). O texto completo também segue no campo *observação* do CA.
