...
> **Preço personalizado por item (09/2026):** no passo 5/6, ao lado de cada item da lista aparece o preço em campo editável — toque no preço (ou no lápis ✎), digite o novo valor e aperte Enter (ou toque fora). Só é permitido **aumentar**: o preço calculado pela condição é o **piso**; se digitar abaixo dele, o campo fica vermelho enquanto digita e volta ao piso sozinho ao sair. Item alterado ganha fundo amarelo claro e o selo **"personalizado"**, com o preço do piso riscado embaixo; o **×** ao lado desfaz aquele item. Trocar de condição de pagamento **recalcula o piso na hora**: se o preço digitado ficar igual ou abaixo do novo piso, o item volta a usar o piso e ganha o selo **"subiu p/ o piso"** (o valor digitado fica lembrado, caso volte a escolher a condição antiga). Abaixo do total aparece uma caixa amarela "N itens com preço personalizado · R$ X a mais" com o botão **"desfazer todos"**. O **cliente não vê** que o preço foi ajustado — a página pública mostra o preço final normalmente, igual a qualquer outro item. Trocar de destinatário (cliente/outro) limpa os preços personalizados digitados. O backend recusa qualquer preço abaixo do piso mesmo que a tela deixe passar (proteção dupla).
...
> **Catálogos gerados pela Ana (09/2026):** a mesma funcionalidade também é acionada pela IA de atendimento do WhatsApp (a "Ana"), via `POST /api/ia-consulta/v1/catalogo/gerar` — ela identifica o cliente pelo telefone de quem manda a mensagem (nunca por CPF/CNPJ) e monta o mesmo tipo de lista, com todos os produtos do catálogo ou uma seleção. Esses catálogos ficam gravados no banco com o **vendedor vinculado ao cliente** (`vendedorNome`, não "Ana") e o **título** `"Catálogo · Ana · <data>"` quando a Ana não informa um título próprio (cliente sem vendedor vinculado → "Hardt Salgados"), e são consultáveis pela própria API (`GET /catalogo/:token`, `POST /catalogo/listar`). **Hoje não existe tela no app que os liste** — ninguém consome os `GET`/`DELETE` de `/api/catalogo-personalizado` para eles (pendente). Ver `backend/docs/ia-consulta-api.md`, seção "Catálogo personalizado (v1.6.5)".
