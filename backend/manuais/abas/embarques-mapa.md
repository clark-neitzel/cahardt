...
- **Marca cinza `FORA` embaixo do pino** — o item está numa carga que você **tirou da divisão automática** (caixa "Participa da divisão" desmarcada no cartão da carga). O pino também fica levemente esmaecido, mas quem identifica o caso é a marca `FORA`: só o esmaecido se confundia com o pino travado.
...
- **Sugerir divisão** (botão verde): o sistema propõe uma divisão entre as cargas **marcadas para participar** (veja abaixo). Cada **cliente é uma parada** (pedido, amostra e cobrança dele contam como uma só e vão para a mesma carga). A proposta **não grava nada** — os pinos recolorem como rascunho. Embaixo do botão a tela avisa quantas cargas vão dividir ("A sugestão divide entre 2 de 3 cargas — a desmarcada fica como está").
  - Depois do cálculo, o painel azul **"Como a sugestão foi montada"** explica o critério (equilibrar o tempo das rotas), o que ele considera e o que não considera, e **quantas cargas participaram** — dizendo também quantas ficaram de fora e não foram alteradas.
...
- **Cartão de cada carga**: bolinha da cor, número e motorista, a contagem do que vai ("14 pedidos · 2 amostras · 1 cobrança"), km/duração/volta prevista com o selo de precisão ("preciso" ou "≈ aproximado"), a caixa **"Participa da divisão"**, aviso âmbar **"impressa na vN — reimprimir"** quando a carga mudou depois da última impressão do romaneio, e o link "abrir carga" (leva ao Painel de Expedição).
  - **"Participa da divisão" (caixa de seleção):** decide se aquela carga entra no rateio automático do **Sugerir divisão**. **Todas nascem marcadas** — desmarque a que não deve receber entregas. É para o caso comum de **carga dedicada**: alguém da empresa que vai levar **um pedido específico** e não pode receber mais nada no caminho.
    ...
    - **O "Trocar tudo desta rota" ignora a marcação.** Ele é um comando manual: se você mandar trocar com uma carga que está **fora da divisão**, a rota inteira vai para lá do mesmo jeito. A caixa "Participa da divisão" protege a carga **só** da sugestão automática — nunca do que você faz à mão.
