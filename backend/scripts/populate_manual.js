...
require('dotenv').config({ path: '../.env.production' }); // Tenta ler o .env.production do backend

// Se não achar, tenta ler do .env normal ou usar a string hardcoded se o usuário permitir (mas não tenho a string real além do que li)
// Li: prisma+postgres://localhost:51213/?api_key=...
// Vou usar a string lida do arquivo .env.production diretamente
...
async function main() {
    console.log('Iniciando população manual...');
    let count = 0;
    for (const p of produtos) {
        await prisma.produto.upsert({
            ...
            create: {
                ...
                nome: p.name,
                precoVenda: p.value,
                saldoEstoque: p.available_stock, // available_stock match
                unidade: p.unity_measure,
                ativo: true
            }
        });
        count++;
        console.log(`Produto processado: ${p.name}`);
    }
    console.log(`Concluído! ${count} produtos processados.`);
}
