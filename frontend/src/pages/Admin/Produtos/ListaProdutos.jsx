...
const ListaProdutos = () => {
    ...
    useEffect(() => {
        const fetchProdutos = async () => {
            ...
            try {
                ...
                if (selectedCategories.length > 0) params.categorias = selectedCategories.join(',');
                if (selectedCatsComerciais.length > 0) params.categoriaProdutoIds = selectedCatsComerciais.join(',');
        ...
        const timeoutId = setTimeout(fetchProdutos, 300);
        return () => clearTimeout(timeoutId);
    }, [page, search, statusFilter, selectedCategories, selectedCatsComerciais]);
    ...
    const handleCatComercialChange = (ids) => {
        setSelectedCatsComerciais(ids);
        setPage(1);
    };

    const getImageUrl = (produto) => {
        if (produto.imagens && produto.imagens.length > 0) {
            return `${API_URL}${produto.imagens[0].url}`;
        }
        return null;
    };

    const renderCategoriaBadge = (produto) => {
        if (!produto.categoriaProduto) return null;
        const cor = produto.categoriaProduto.corTag;
        return (
            <span
                className="inline-block px-2 py-0.5 rounded-full text-xs font-medium mt-1"
                style={cor ? { backgroundColor: `${cor}22`, color: cor, border: `1px solid ${cor}55` } : {}}
            >
                ...
                <div className="p-4 border-b border-gray-200 bg-gray-50 flex flex-col gap-3">
                    ...
                    <div className="flex flex-col md:flex-row gap-3">
                        ...
                        <div className="w-full md:w-64 z-10">
                            <MultiSelect
                                options={availableCatsComerciais}
                                selected={selectedCatsComerciais}
                                onChange={handleCatComercialChange}
                                placeholder="Filtrar por Cat. Comercial"
                                valueKey="id"
                                labelKey="nome"
                            />
                        </div>
                ...
                <div className="md:hidden divide-y divide-gray-200">
                    {loading ? (
                        ...
                        produtos.map((produto) => {
                            const imgUrl = getImageUrl(produto);
                            return (
                                <div
                                    key={produto.id || produto.uuid}
                                    onClick={() => navigate(`/admin/produtos/${produto.id || produto.uuid}`, { state: navState })}
                                    className="p-4 active:bg-gray-50 flex items-center justify-between cursor-pointer"
                                >
                                    <div className="flex items-center flex-1 min-w-0">
                                        <div className="h-10 w-10 flex-shrink-0">
                                            {imgUrl ? (
                                                <img className="h-10 w-10 rounded object-cover" src={imgUrl} alt="" />
                                            ) : (
                                        ...
                                        <div className="ml-3 flex-1 min-w-0">
                                            ...
                                            {produto.categoriaProduto && (
                                                <div>{renderCategoriaBadge(produto)}</div>
                                            )}
