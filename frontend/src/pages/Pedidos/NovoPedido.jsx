...
const NovoPedido = () => {
                                    ...
                                    <div className="mt-1 text-xs bg-white/70 border border-green-100 rounded px-2 py-1.5 space-y-1">
                                        {g.condicoes?.map((c, ci) => {
                                            const nomeProd = c.tipo === 'PRODUTO_QUANTIDADE'
                                                ...
                                                : null;
                                            // Condição já atingida? (✓ verde / círculo cinza)
                                            const cumprida = c.tipo === 'PRODUTO_QUANTIDADE'
                                                ? Number(itensMap.get(c.produtoId)?.quantidade || 0) >= Number(c.quantidadeMinima)
                                                : vTotal >= Number(c.valorMinimo);
                                            const StatusIcon = cumprida
                                                ...
                                                <div key={c.id}>
                                                    ...
                                                    {c.tipo === 'PRODUTO_QUANTIDADE' ? (
                                                        <div className="flex items-center justify-between gap-2">
                                                            <span className="flex items-center gap-1.5 min-w-0">{StatusIcon}<span className="text-gray-700 font-medium truncate">{nomeProd}</span></span>
                                                            <span className={`font-bold shrink-0 ${cumprida ? 'text-green-700' : 'text-purple-700'}`}>≥ {c.quantidadeMinima} un</span>
                                                        </div>
                                                    ) : (
                                                        <div className="flex items-center justify-between gap-2">
                                                            <span className="flex items-center gap-1.5 min-w-0">{StatusIcon}<span className="text-gray-700 font-medium">Valor do pedido</span></span>
                                                            <span className={`font-bold shrink-0 ${cumprida ? 'text-green-700' : 'text-purple-700'}`}>≥ R$ {Number(c.valorMinimo).toFixed(2).replace('.', ',')}</span>
                                                        </div>
                                                    )}
                                                </div>
                                            );
    ...
    return (
        <div className="bg-gray-50 min-h-screen flex flex-col">
            ...
            {/* ===== CORPO: 2 colunas no desktop (formulário+produtos | carrinho) ===== */}
            <div className="flex-1 flex flex-col min-h-0 lg:grid lg:grid-cols-[minmax(0,1fr)_22rem] lg:gap-4 lg:px-4 lg:py-4 lg:items-start">
            {/* COLUNA ESQUERDA — display:contents no mobile (não muda nada); vira coluna no desktop */}
            <div className="contents lg:flex lg:flex-col lg:gap-4 lg:min-w-0">
            {/* ===== FORMULÁRIO (cliente + etapas sequenciais) ===== */}
            <div className="bg-white shadow-sm lg:rounded-xl lg:border lg:border-gray-200">
                ...
                {clienteId && (
                    <>
                        ...
                        {mostrarFormulario && (
                            <div className={`px-3 pt-1 space-y-3 border-t border-gray-100 lg:pb-4 ${itensMap.size > 0 ? 'pb-36' : 'pb-3'}`}>
            ...
            {/* ===== LISTA DE PRODUTOS ===== */}
            {clienteId && tipoPedido && condicaoPagamentoId && canalOrigem && (
                <div className={`flex-1 pb-40 lg:pb-2 ${mostrarFormulario ? 'hidden lg:block' : ''}`}>
            ...
            {/* ===== COLUNA DIREITA — CARRINHO (somente desktop) ===== */}
            {clienteId && tipoPedido && condicaoPagamentoId && canalOrigem && (
                <aside className="hidden lg:flex lg:flex-col lg:sticky lg:top-20 lg:max-h-[calc(100vh-6rem)] bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    <div className="px-4 py-3 border-b border-gray-100">
                        <div className="flex items-center gap-2">
                            <div className="w-9 h-9 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center text-xs font-bold shrink-0">
                                {(clienteSelecionado?.NomeFantasia || clienteSelecionado?.Nome || '?').slice(0, 2).toUpperCase()}
                            </div>
                        ...
                        <div className="flex flex-wrap gap-1.5 mt-2 text-[11px]">
                            <span className="inline-flex items-center gap-1 bg-gray-50 border border-gray-200 rounded-md px-2 py-0.5 text-gray-600">
                                <Calendar className="h-3 w-3" />{dataEntrega ? dataEntrega.split('-').reverse().join('/') : '—'}
                            </span>
                            {condicaoSelecionada && (
                                <span className="bg-gray-50 border border-gray-200 rounded-md px-2 py-0.5 text-gray-600">
                                    {condicaoSelecionada.nomeCondicao} ({condicaoSelecionada.acrescimoPreco}%)
                                </span>
                            )}
                            {canalOrigem && (
                                <span className="bg-gray-50 border border-gray-200 rounded-md px-2 py-0.5 text-gray-600">
                                    {TIPOS_ATENDIMENTO.find(t => t.value === canalOrigem)?.label}
                                </span>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="px-4 py-2 flex items-center justify-between border-b border-gray-100">
                        <span className="text-xs font-bold text-gray-500 uppercase tracking-wide">Itens do Pedido</span>
                        <span className="text-xs font-bold text-white bg-blue-600 rounded-full px-2 py-0.5">{itensMap.size}</span>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto divide-y divide-gray-100">
                        {itensMap.size === 0 ? (
                            <div className="px-4 py-10 text-center text-gray-400">
                                ...
                                <p className="text-xs">Nenhum item ainda.<br />Adicione produtos à esquerda.</p>
                            </div>
                        ) : Array.from(itensMap.entries()).map(([pid, it]) => {
                            const prod = produtos.find(p => p.id === pid);
                            const nome = prod?.nome || pid;
                            const linha = Number(it.valorUnitario) * Number(it.quantidade);
                            const permiteFracao = !!prod?.categoriaProduto?.permiteFracao;
                            return (
                                <div key={pid} className="px-4 py-2.5">
                                    <div className="flex items-start justify-between gap-2">
                                        <p className="text-[13px] font-semibold text-gray-800 leading-tight line-clamp-2">{nome}</p>
                                    ...
                                    <div className="flex items-center justify-between mt-1.5">
                                        <div className="flex items-center gap-1">
                                            <button
                                                onClick={() => {
                                                    const step = permiteFracao ? 0.1 : 1;
                                                    const next = Math.round((it.quantidade - step) * 1000) / 1000;
                                                    setQuantidade(pid, next > 0 ? next : 0);
                                                }}
                                                className="w-7 h-7 flex items-center justify-center rounded-md bg-gray-100 text-gray-600 hover:bg-gray-200"
                                            >
                                                <Minus className="h-3.5 w-3.5" />
                                            </button>
                                            <QuantidadeInput value={it.quantidade} permiteFracao={permiteFracao} onChange={v => setQuantidade(pid, v)} />
                                            <button
                                                onClick={() => setQuantidade(pid, it.quantidade + 1)}
                                                className="w-7 h-7 flex items-center justify-center rounded-md bg-blue-600 text-white hover:bg-blue-700"
                                            >
                                                <Plus className="h-3.5 w-3.5" />
                                            </button>
                                        </div>
                                        <button
                                            onClick={() => setQuantidade(pid, 0)}
                    ...
                    {/* Totais + Fechar pedido */}
                    <div className="border-t border-gray-200 px-4 py-3 space-y-1.5">
                        <div className="flex justify-between text-xs text-gray-500">
                            <span>Subtotal</span>
                            <span>R$ {(bonificacao ? 0 : vTotal).toFixed(2).replace('.', ',')}</span>
                        </div>
                        {Number(valorFrete) > 0 && (
                            <div className="flex justify-between text-xs text-gray-500">
                                <span>Frete</span>
                                <span>R$ {Number(valorFrete).toFixed(2).replace('.', ',')}</span>
                            </div>
                        )}
                        {!bonificacao && (
                            <div className="flex justify-between text-xs">
                                <span className="text-gray-500">Flex</span>
                                <span className={`font-semibold ${flexTotal >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                    {flexTotal > 0 ? '+' : ''}{flexTotal.toFixed(2).replace('.', ',')}
                                </span>
                            </div>
                        )}
                        <div className="flex justify-between items-center pt-1.5 border-t border-gray-100">
                            <span className="text-sm font-bold text-gray-900">Total</span>
                            <span className="text-lg font-extrabold text-gray-900">R$ {(bonificacao ? 0 : vTotal).toFixed(2).replace('.', ',')}</span>
                        </div>
                        {itensMap.size > 0 && (() => {
                            ...
                            if (bloqueado) {
                                return (
                                    <div className="space-y-2 pt-1">
                                        <div className="bg-red-50 text-red-700 px-2.5 py-1.5 rounded-md text-[11px] font-semibold flex items-center gap-1.5 border border-red-100">
                                            <AlertCircle className="h-3.5 w-3.5 shrink-0" />
                                            <span>Faltam R$ {(min - vTotal).toFixed(2).replace('.', ',')} para o mínimo (R$ {min.toFixed(2).replace('.', ',')})</span>
                                        </div>
                                        <button disabled className="w-full bg-gray-300 text-gray-500 font-bold py-2.5 rounded-lg text-xs cursor-not-allowed uppercase tracking-wide">
                            ...
                            return (
                                <button
                                    ...
                                    disabled={saving}
                                    className="w-full mt-1 bg-green-600 hover:bg-green-700 text-white font-bold py-2.5 rounded-lg shadow-sm text-sm disabled:opacity-50 transition-colors flex items-center justify-center gap-2"
                                >
                                    <CheckCircle className="h-4 w-4" />
                                    {saving ? 'Enviando...' : bonificacao ? 'Fechar bonificação' : 'Fechar pedido'}
                                </button>
                            );
                        })()}
                    </div>
            ...
            {/* ===== FOOTER FECHAR PEDIDO (somente mobile) ===== */}
            {itensMap.size > 0 && (
                <div className="fixed bottom-0 left-0 right-0 p-3 bg-white border-t border-gray-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-20 lg:hidden">
                    {(() => {
                        const min = Number(condicaoSelecionada?.valorMinimo) || 0;
                        const bloqueado = min > 0 && vTotal < min;
