/**
 * Utilitário único de NOME DE CIDADE (padronização de grafia) — ESPELHO DO BACKEND.
 * Fonte: backend/utils/cidade.js — mantenha os dois em sincronia (mesmo dicionário,
 * mesmas regras). Aqui é ES Module (import/export); lá é CommonJS.
 *
 * Extra do frontend: `chaveBusca` — a MESMA normalização de `chaveCidade` (sem acento,
 * minúscula, espaços colapsados), exportada com nome genérico porque os dropdowns do app
 * (SelectBusca / MultiSelect / ComboBusca) usam-na para filtrar QUALQUER texto:
 * digitar "itapoa" acha "Itapoá", "sao" acha "São".
 *
...
/** Normalização genérica de texto para BUSCA nos dropdowns (idêntica a `chaveCidade`). */
const chaveBusca = chaveCidade;
