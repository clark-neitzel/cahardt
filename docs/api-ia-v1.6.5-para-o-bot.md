# API de Consulta do CA-Hardt para a IA (v1.6.5) — guia autocontido para o time do bot
...
como autenticar, todos os endpoints (histórico completo da v1.0 até a v1.6.5), o formato do objeto
...
```json
{
  "meta": { "versaoApi": "1.6.5", "avisos": [], "geradoEm": "2026-09-16T12:00:00.000Z" },
...
## 3. Tabela de endpoints (histórico completo v1.0 → v1.6.5)
...
| POST | `/cliente/buscar` | 1.5 (+1.6.2) | `{ busca, limite? }` (mín. 3 caracteres) | 🔒 Só painel interno — busca **cliente E fornecedor** por nome/documento (seção 9) |
| POST | `/cliente/ficha` | 1.5 (+1.6.2) | `{ documento }` | 🔒 Só painel interno — ficha completa de um cliente **ou fornecedor** (seção 9) |
| POST | `/cliente/adicionar-whatsapp` | 1.6.3 | `{ documento, whatsapp, origem? }` (`origem`: `"painel-bot"` padrão ou `"ana"`) | 🔒 Só painel interno — grava um WhatsApp no cadastro do cliente, gravação atômica (seção 9) |
...
| POST | `/catalogo/gerar` | 1.6.5 | `{ telefone, produtoIds?:[...], todos?:boolean, titulo?, observacoes?, condicaoId?, idempotencyKey? }` | **PODE ser tool da IA.** Monta uma lista de preços pro cliente reconhecido e devolve o link (seção 9.5) |
| GET | `/catalogo/:token` | 1.6.5 | — | **PODE ser tool da IA.** Relê um catálogo já gerado pelo token (sem telefone — o token já é o segredo) |
| POST | `/catalogo/listar` | 1.6.5 | `{ telefone, limite? }` | **PODE ser tool da IA.** Catálogos ativos do cliente reconhecido |
...
4. `/cliente/buscar`, `/cliente/ficha`, `/cliente/situacao` e `/cliente/adicionar-whatsapp` são
   **só para o painel interno da equipe humana** — nunca tools que a Ana usa para responder cliente.
...
- **(v1.6.2, NOVO) `POST /cliente/buscar` e `POST /cliente/ficha` agora também buscam em
  Fornecedor**, não só em Cliente — antes o painel não achava uma empresa que só existe como
  fornecedor (ex.: "Karville"), porque a busca só olhava a tabela de clientes.
  - `cliente/buscar`: cada item da lista `clientes[]` ganha o campo `tipo` (`"CLIENTE"` ou
    `"FORNECEDOR"`). Fornecedor usa o **mesmo formato** do item de cliente
    (`documento, nome, nomeFantasia, cidade, vendedor, ativo, telefones[], whatsapps[]`), com
    `vendedor` sempre `null` e `whatsapps` sempre `[]` (cadastro de fornecedor não tem essas
    colunas). Ordem: clientes primeiro, fornecedores depois; `limite` conta o total combinado. Se
    o mesmo documento existir nos dois cadastros, os dois itens aparecem (com `tipo` diferente).
  - `cliente/ficha`: ganha o campo `tipo`. Se o documento **não** é de cliente, a API procura em
    Fornecedor antes de devolver `{ "encontrado": false }` — mesmo shape, com
    `diasEntrega`/`diasVenda` vazios e `condicaoPagamento`/`vendedor`/`horaCorte` `null`
    (fornecedor não tem essas informações — `horaCorte:null` é mantido de propósito, só para o
    shape ficar igual ao da ficha de cliente, que sempre traz esse campo), mais um objeto novo
    `fornecedor: { email?, telefone?, inscricaoEstadual?, uf? }` só com o que existir no cadastro.
    Se o documento existir **como cliente e fornecedor ao mesmo tempo**, o **cliente tem
    prioridade** (é o que volta) e a resposta ganha `"tambemFornecedor": true`.
  - **Tudo aditivo** — nenhum campo removido/renomeado nesses dois endpoints.
...
- **(v1.6.3, NOVO) `POST /cliente/adicionar-whatsapp`**: 🔒 **nunca é tool da IA.** Existe para o
  painel gravar, no cadastro do CA-Hardt, o número de WhatsApp de uma conversa que a equipe acabou
  de vincular manualmente a um cliente — depois disso, o reconhecimento automático por telefone
  (`cliente/reconhecer-telefone` e `congelados/reconhecer-telefone`) passa a casar sozinho da
  próxima vez que esse número mandar mensagem, sem precisar vincular de novo. **Não libera nenhum
  dado do cliente** (não é consulta, é só gravação) — por isso não fere a regra da seção 4 de nunca
  identificar cliente só por CPF/CNPJ: quem decide gravar é a equipe logada no painel, não a IA.

  Body: `{ "documento": "12345678000190", "whatsapp": "47999991234", "origem": "painel-bot" }`
  (`origem` opcional, padrão `"painel-bot"` — vai para a auditoria interna do CA-Hardt).

  Regras:
  - **Só ACRESCENTA.** Nunca apaga nem substitui um WhatsApp já salvo no cadastro.
  - `whatsapp` é normalizado com a **mesma função da tela de Clientes do app** (não é uma regra
    própria deste endpoint): só tira o que não é dígito — **não retira o DDI 55** — e precisa
    sobrar entre 10 e 13 dígitos, senão `400 WHATSAPP_INVALIDO`. Mandar `"5547999991234"` grava
    exatamente `"5547999991234"` (com o 55) — é assim que o número aparece na tela de Clientes,
    então grava igual não importa quem gravou.
  - Se o número já estiver na lista de WhatsApps do cliente **ou** já for o telefone/celular do
    cadastro (mesma tolerância de sempre: com/sem 9º dígito, com/sem DDI 55), a chamada é
    **idempotente**: não duplica, só devolve `jaExistia:true`. Seguro repetir a chamada.
  - **Gravação atômica**: duas chamadas simultâneas com números diferentes para o mesmo cliente não
    se atropelam (o CA-Hardt grava com um `array_append` dentro do próprio comando SQL, não
    lendo-e-regravando a lista inteira). Se cair exatamente no mesmo instante que outra chamada
    gravando o MESMO número, o resultado ainda é `jaExistia:true` (sem duplicar).
  - Limite de 10 números por cliente — acima disso, `400 LIMITE_WHATSAPPS`.
  - Documento que não existe como cliente: `404 CLIENTE_NAO_ENCONTRADO`. Documento que existe só
    como **fornecedor**: `400 NAO_E_CLIENTE` (este endpoint só grava em cadastro de cliente).
  - `origem` é uma **lista fechada**: só `"painel-bot"` (padrão) ou `"ana"`. Qualquer outro valor
    devolve `400 ORIGEM_INVALIDA` — não é texto livre, é usado na auditoria interna.

  Resposta: `{ "ok": true, "jaExistia": false, "tipo": "CLIENTE", "numeroGravado": "5547999991234" }`.

---

## 9.5 Catálogo personalizado (v1.6.5) — a Ana monta uma lista de preços pro cliente

**Diferente da seção 9 (que é só painel), estes três endpoints PODEM ser tool da Ana** — é uma ação
transacional provocada pelo próprio cliente na conversa ("me manda os preços", "tem lista dos
salgados?"), igual à criação de pedido (seção 8). Reaproveita a mesma funcionalidade que o vendedor
já usa na tela Produtos → Catálogo do app: um **snapshot** de produtos com preço já calculado pela
condição do cliente, publicado numa página pública (`https://hardtsalgados.com.br/lista/<token>`,
sem login, válida por 7 dias).

### `POST /catalogo/gerar`

Body: `{ telefone, produtoIds?:[...], todos?:boolean, titulo?, observacoes?, condicaoId?, idempotencyKey? }`

- **Identificação SEMPRE por `telefone`** — mesma regra da seção 4 (nunca CPF/CNPJ sozinho: o link
  mostra preço negociado). Telefone que não bate com cadastro → `{ "reconhecido": false }` — não
  gerar catálogo pra desconhecido (não existe preço negociado pra calcular).
- **Produtos** — um dos dois é obrigatório:
  - `produtoIds: [...]` — aceita tanto o `produtoId` que já vem nos objetos de produto desta API
    quanto o `id` do catálogo do site de Congelados (o mesmo `id` de `GET /congelados/catalogo`) —
    o CA-Hardt resolve os dois formatos automaticamente, pode mandar qualquer um. Cada id passa
    pelo mesmo filtro de "produto de venda" do `todos:true` — id que não é (ferragem, insumo,
    inativo) é ignorado em silêncio, não dá erro por si só.
  - `todos: true` — todo o catálogo de venda (produto ativo, dentro das categorias configuradas
    pro catálogo, sem imobilizado). Bom para "manda a lista completa" / "manda o catálogo".
  - Nenhum dos dois, ou nenhum produto disponível → `400 { "code": "SEM_PRODUTOS" }`.
- **`condicaoId`** opcional — sem informar, usa a condição de pagamento **padrão do cliente** (a
  mesma que vem em `reconhecer-telefone` → `condicaoPagamento.id`). Se informar uma condição
  inválida ou não disponível para catálogo → `400 { "code": "CONDICAO_INVALIDA" }`. Cliente sem
  condição padrão e sem `condicaoId` → `400 { "code": "SEM_CONDICAO" }`. Não precisa validar antes
  de chamar — só usar o `id`/`prazoDias`/... que já veio de `reconhecer-telefone`, se o cliente
  pedir uma condição diferente da padrão explicitamente.
- **`titulo`** opcional — sem informar, sai como `"Catálogo · Ana · <data>"`.
- **`idempotencyKey`** opcional, aceita mas **não é o que evita duplicata** — o CA-Hardt já reaproveita
  sozinho o catálogo idêntico dos últimos 10 minutos (telefone + condição + mesmo conjunto de
  produtos), devolvendo `"reaproveitado": true` em vez de criar outro. **Isso não é uma trava
  perfeita** (é checagem + gravação, sem lock): num retry disparado quase ao mesmo tempo da primeira
  chamada — antes dela terminar —, raramente, o CA-Hardt pode gerar dois catálogos em vez de um. Na
  prática o efeito é só um link a mais válido, nada financeiro. **Pelo lado do bot:** depois que
  `gerar` responder com sucesso, guardar o `token`/`link` devolvido e não chamar `gerar` de novo pro
  mesmo pedido do cliente sem necessidade — não contar com o dedupe do servidor como garantia.

Resposta (sucesso, cliente reconhecido):
```json
{
  "reconhecido": true,
  "id": "uuid",
...
```
`vendedorNome` é o vendedor vinculado ao cliente (ou `"Hardt Salgados"` se não tiver). Não confundir
com "a Ana mandou" — quem aparece como remetente da lista é sempre o vendedor do cliente, igual à
tela do app.

### `GET /catalogo/:token`

Sem telefone — o token já é o segredo, igual ao link público que o cliente recebe. Devolve o mesmo
formato da página pública (`titulo`, `clienteNome`, `condicaoNome`, `validadeEm`, `expirado`,
`vendedorNome`, `whatsapp`, `itens[]` com `precoFinal`). Útil pra Ana reler/conferir o que já
mandou sem guardar o conteúdo em memória própria. Não encontrado/removido →
`404 { "code": "CATALOGO_NAO_ENCONTRADO" }`.

### `POST /catalogo/listar`

Body: `{ telefone, limite? }` (padrão 10, máx 50). Catálogos ainda ativos do cliente reconhecido,
mais recentes primeiro: `{ reconhecido, cliente:{nome}, catalogos:[{ id, token, link, titulo,
condicaoNome, total, qtdItens, validadeEm, visualizacoes, criadoEm }] }`. Útil se o cliente disser
...
# busca no painel (agora inclui fornecedor) — exemplo: achar "Karville"
curl -H "$K" -H "$J" -X POST -d '{"busca":"karville","limite":10}' \
  $B/cliente/buscar | jq '.dados.clientes[] | {tipo,nome,documento}'

# ficha de fornecedor pelo documento
curl -H "$K" -H "$J" -X POST -d '{"documento":"<cnpj do fornecedor>"}' \
  $B/cliente/ficha | jq '.dados'

# v1.6.3 — painel vincula o WhatsApp da conversa ao cadastro do cliente
curl -H "$K" -H "$J" -X POST -d '{"documento":"12345678000190","whatsapp":"5547999991234","origem":"painel-bot"}' \
  $B/cliente/adicionar-whatsapp | jq '.dados'

# v1.6.5 — Ana gera catálogo com TODOS os produtos pro cliente reconhecido
curl -H "$K" -H "$J" -X POST -d '{"telefone":"5547999998888","todos":true}' \
  $B/catalogo/gerar | jq '.dados | {link,titulo,total,qtdItens}'

# v1.6.5 — Ana gera catálogo com produtos específicos
curl -H "$K" -H "$J" -X POST -d '{"telefone":"5547999998888","produtoIds":["<id1>","<id2>"],"titulo":"Sugestão da semana"}' \
  $B/catalogo/gerar | jq '.dados'

# v1.6.5 — reler um catálogo pelo token
curl -H "$K" "$B/catalogo/<token>" | jq '.dados'

# v1.6.5 — listar catálogos já mandados pro cliente
curl -H "$K" -H "$J" -X POST -d '{"telefone":"5547999998888"}' \
  $B/catalogo/listar | jq '.dados.catalogos'
```

---

## 13. Avisos ativos (histórico até 2026-09-19, v1.6.5)
...
Nenhum aviso pendente do `POST /cliente/buscar`/`POST /cliente/ficha` (v1.6.2) — é aditivo, não
tem prazo de remoção associado. **Estes dois endpoints são só painel** (seção 9) — a mudança não
afeta nada que a Ana usa para responder o cliente.

Nenhum aviso pendente do `POST /cliente/adicionar-whatsapp` (v1.6.3) — é um endpoint 100% novo,
não altera nenhum campo de resposta existente. **Também é só painel** (seção 9).

Nenhum aviso pendente de `POST /catalogo/gerar`, `GET /catalogo/:token` e `POST /catalogo/listar`
(v1.6.5) — são endpoints 100% novos, nenhum campo de resposta existente foi alterado. **Estes,
diferente dos anteriores, PODEM ser tool da Ana** (seção 9.5) — são ação transacional a pedido do
cliente, igual à criação de pedido.
